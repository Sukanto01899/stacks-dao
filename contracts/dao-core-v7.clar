;; Simple DAO core v7 (testing-only): balance-based voting, treasury execution, and timelock.
...
(define-constant PROPOSAL_THRESHOLD u0)
...
(define-private (get-voting-power (who principal))
  (stx-get-balance who)
)
...
(define-read-only (get-voting-power-balance (who principal))
...
(define-public (propose
  ...
  (let (
      ...
      (power (get-voting-power tx-sender))
      (now stacks-block-height)
      (last-end (var-get last-proposal-end-height))
      )
      (if (<= now last-end)
      (err ERR_ACTIVE_PROPOSAL)
